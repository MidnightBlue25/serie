...
import { Serie } from '../serie/entity/serie.entity.js';
import { entities } from '../serie/entity/entities.js';
...
const database = (db?.name as string | undefined) ?? Serie.name.toLowerCase();

const host = (db?.host as string | undefined) ?? 'localhost';
const username =
    (db?.username as string | undefined) ?? Serie.name.toLowerCase();
