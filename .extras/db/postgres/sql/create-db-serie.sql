...
-- (3) In Windows das Verzeichnis "C:/Zimmermann/volumes/postgres/tablespace/serie" anlegen
--     Siehe .extras\compose\postgres\compose.yml
-- (4) 2. PowerShell:
--     cd .extras\compose\backend\postgres
--     docker compose exec db bash
--        chown postgres:postgres /var/lib/postgresql/tablespace
--        chown postgres:postgres /var/lib/postgresql/tablespace/serie
...
-- (7) 2. PowerShell:
--     docker compose exec db bash
--        psql --dbname=postgres --username=postgres --file=/sql/create-db-serie.sql
--        psql --dbname=serie --username=serie --file=/sql/create-schema-serie.sql
--        exit
...
CREATE ROLE serie LOGIN PASSWORD 'p';

-- https://www.postgresql.org/docs/current/sql-createdatabase.html
CREATE DATABASE serie;

GRANT ALL ON DATABASE serie TO serie;

-- https://www.postgresql.org/docs/10/sql-createtablespace.html
CREATE TABLESPACE seriespace OWNER serie LOCATION '/var/lib/postgresql/tablespace/serie';
